''' This use for Reference stage ---> '''
from time import time
import torch
import torchvision
from .postprocessing.ConvertYolo2Pix import Yolo2Pix



''' 13/10/2025: still exist fixed threshhold - iou'''
def non_max_suppression(outputs, confidence_threshold=0.001, iou_threshold=0.65):
    ...
    for index, x in enumerate(outputs):  # image index, image inference
        ...
        box = Yolo2Pix(box)  # (cx, cy, w, h) to (x1, y1, x2, y2)
        if nc > 1:
